<?php if($_SERVER['REQUEST_URI'] != '/login'){ header('Location: /not-found'); exit(); } require_once $_SERVER['DOCUMENT_ROOT'].'/src/database/db.php'; require_once $_SERVER['DOCUMENT_ROOT'].'/src/functions/includeAsset.php'; if($auth -> isLoggedIn()){ header('Location: /dashboard'); exit(); } require_once $_SERVER['DOCUMENT_ROOT'].'/inc/head/content.phtml'; require_once $_SERVER['DOCUMENT_ROOT'].'/inc/layouts/header/content.phtml'; ?> 
<main>
