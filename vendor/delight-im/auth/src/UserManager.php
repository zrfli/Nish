...
abstract class UserManager {
	 ...
	 */
	protected function createUserInternal($requireUniqueUsername, $email, $password, $gender, $country, $username = null, callable $callback = null) {
		...
		if ($requireUniqueUsername) {
			...
			if ($username !== null) {
				...
				$occurrencesOfUsername = $this->db->selectValue(
					'SELECT COUNT(*) FROM ' . $this->makeTableName('Ms_Users') . ' WHERE username = ?',
					[ $username ]
				);
        ...
        $gen = $gender;
        $nationality = mb_strtolower($country);
	...
	protected function getUserDataByUsername($username, array $requestedColumns) {
		try {
			...
			$users = $this->db->select(
				'SELECT ' . $projection . ' FROM ' . $this->makeTableName('Ms_Users') . ' WHERE username = ? LIMIT 2 OFFSET 0',
				[ $username ]
			);
		}
		catch (Error $e) {
	...
	protected function forceLogoutForUserById($userId) {
		...
		$this->db->exec(
			'UPDATE ' . $this->makeTableName('Ms_Users') . ' SET force_logout = force_logout + 1 WHERE id = ?',
			[ $userId ]
		);
	}
