...
final class Auth extends UserManager {
				...
				if (!empty($parts[0]) AND !empty($parts[1])) {
					try {
						$rememberData = $this->db->selectRow(
							'SELECT a.user, a.token, a.expires, b.email, b.username, b.idenity_number, b.first_name, b.last_name, b.gender, b.phone_number, b.birthday, b.address, b.twofa_authentication, b.avatar, b.academic_info, b.verification, b.status, b.roles_mask, b.force_logout FROM ' . $this->makeTableName('Ms_Users_Remembered') . ' AS a JOIN ' . $this->makeTableName('Ms_Users') . ' AS b ON a.user = b.id WHERE a.selector = ?',
							[ $parts[0] ]
						);
					}
					catch (Error $e) {
					...
					if (!empty($rememberData)) {
						if ($rememberData['expires'] >= \time()) {
							if (\password_verify($parts[1], $rememberData['token'])) {
								...
								$_SESSION['username'] = mb_strtolower($rememberData['username']);
								$_SESSION['idenity'] = $rememberData['idenity_number'];
								$_SESSION['firstName'] = mb_convert_case($rememberData['first_name'], MB_CASE_TITLE, "UTF-8");
								$_SESSION['lastName'] = mb_convert_case($rememberData['last_name'], MB_CASE_TITLE, "UTF-8");
								$_SESSION['birthday'] = $rememberData['birthday'];
								$_SESSION['avatar'] = $rememberData['avatar'];
								$_SESSION['gender'] = $rememberData['gender'];
								$_SESSION['phoneNumber'] = $rememberData['phone_number'];
								$_SESSION['address'] = $rememberData['address'];
								$_SESSION['status'] = (int) $rememberData['status'];

								$objAcademic = json_decode($rememberData['academic_info']);
								if(isset($objAcademic -> tag)){ $_SESSION['academic_details']['tag'] = $objAcademic -> tag ?? NULL; }

								$obj2fa = json_decode($rememberData['twofa_authentication']);
								$_SESSION['2fa']['secretKey'] = (string) $obj2fa -> secret_key;
								$_SESSION['2fa']['status'] = (int) $obj2fa -> status;

								$obj = json_decode($rememberData['verification']);
								$_SESSION['infoVerify'] = (int) $obj -> info;
								$_SESSION['idenityVerify'] = (int) $obj -> idenity;
								$_SESSION['contractsVerify'] = (int) $obj -> contracts;
			...
			if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= \time()) {
				...
				try {
					$authoritativeData = $this->db->selectRow(
						'SELECT username, idenity_number, first_name, last_name, gender, email, phone_number, birthday, address, twofa_authentication, academic_info, avatar, status, verification, roles_mask, force_logout FROM ' . $this->makeTableName('Ms_Users') . ' WHERE id = ?',
						[ $this->getUserId() ]
					);
				} catch (Error $e) { throw new DatabaseError($e->getMessage()); }
				...
				if (!empty($authoritativeData)) {
					...
					if ($authoritativeData['force_logout'] > $_SESSION[self::SESSION_FIELD_FORCE_LOGOUT] OR $authoritativeData['status'] !== 0) {
					...
					// if the counter that keeps track of forced logouts has remained unchanged
                        ...
                        //Custom data
                        $_SESSION['email'] = $authoritativeData['email'];
                        $_SESSION['username'] = mb_strtolower($authoritativeData['username']);
                        $_SESSION['idenity'] = $authoritativeData['idenity_number'];
                        $_SESSION['firstName'] = mb_convert_case($authoritativeData['first_name'], MB_CASE_TITLE, "UTF-8");
						$_SESSION['lastName'] = mb_convert_case($authoritativeData['last_name'], MB_CASE_TITLE, "UTF-8");
                        $_SESSION['birthday'] = $authoritativeData['birthday'];
						$_SESSION['avatar'] = $authoritativeData['avatar'];
                        $_SESSION['gender'] = $authoritativeData['gender'];
                        $_SESSION['phoneNumber'] = $authoritativeData['phone_number'];
                        $_SESSION['address'] = $authoritativeData['address'];
                        $_SESSION['status'] = (int) $authoritativeData['status'];
						
						$objAcademic = json_decode($authoritativeData['academic_info']);
						if(isset($objAcademic -> tag)){ $_SESSION['academic_details']['tag'] = $objAcademic -> tag ?? NULL; }

						$obj2fa = json_decode($authoritativeData['twofa_authentication']);
						$_SESSION['2fa']['secretKey'] = (string) $obj2fa -> secret_key;
						$_SESSION['2fa']['status'] = (int) $obj2fa -> status;

						$obj = json_decode($authoritativeData['verification']);
						$_SESSION['infoVerify'] = (int) $obj -> info;
						$_SESSION['idenityVerify'] = (int) $obj -> idenity;
						$_SESSION['contractsVerify'] = (int) $obj -> contracts;
	 ...
	 */
	public function register($email, $password, $gender, $country, $username = null, callable $callback = null) {
		try {
			$userRow = $this -> db -> selectValue(
				'SELECT COUNT(*) FROM ' . $this -> makeTableName('Ms_Users') . ' WHERE email = ?',
				[ $email ]
			);
			...
			if((int) $userRow == 0){
				...
				$newUserId = $this->createUserInternal(false, $email, $password, $gender, $country, $username, $callback);

				$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 1, (60 * 60 * 12), 5, false);

				$columnsToFetch = [ 'email', 'password', 'verified', 'username', 'status', 'roles_mask', 'force_logout', 'idenity_number', 'first_name', 'last_name', 'gender', 'phone_number', 'birthday', 'address', 'avatar' ];

				$userData = $this -> getUserDataByEmailAddress($email, $columnsToFetch);

				$_SESSION['email'] = $userData['email'];
				$_SESSION['username'] = mb_strtolower($userData['username']);
				$_SESSION['idenity'] = $userData['idenity_number'];
				$_SESSION['firstName'] = mb_convert_case($userData['first_name'], MB_CASE_TITLE, "UTF-8");
				$_SESSION['lastName'] = mb_convert_case($userData['last_name'], MB_CASE_TITLE, "UTF-8");
				$_SESSION['birthday'] = $userData['birthday'];
				$_SESSION['avatar'] = $userData['avatar'];
                $_SESSION['status'] = (int) $userData['status'];
				$_SESSION['gender'] = (int) $userData['gender'];
				$_SESSION['phoneNumber'] = $userData['phone_number'];
				$_SESSION['address'] = $userData['address'];

				$obj = json_decode($userData['verification']);
				$_SESSION['infoVerify'] = (int) $obj -> info;
				$_SESSION['idenityVerify'] = (int) $obj -> idenity;
				$_SESSION['contractsVerify'] = (int) $obj -> contracts;
	 ...
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
	public function loginWithUsername($username, $password, $twoFactorValid, $rememberDuration = null, callable $onBeforeSuccess = null) {
		$this->throttle([ 'attemptToLogin', 'username', $username ], 500, (60 * 60 * 24), null, true);

		$this->authenticateUserInternal($password, null, $username, $rememberDuration, $twoFactorValid, $onBeforeSuccess);
	}
		...
		if ($this->isLoggedIn()) {
			...
			try {
				$expectedHash = $this->db->selectValue(
					'SELECT password FROM ' . $this->makeTableName('Ms_Users') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
			}
			catch (Error $e) {
	...
	public function logOut() {
		...
		if ($this->isLoggedIn()) {
            ...
            unset($_SESSION['idenityVerify']);
			unset($_SESSION['contractsVerify']);
            unset($_SESSION['infoVerify']);
			unset($_SESSION['academic_details']);
			unset($_SESSION['2fa']);

			/*setcookie('test', 'DELETED', time() - 1, '/');*/
		}
	}
	...
	private function createRememberDirective($userId, $duration) {
		...
		try {
			$this->db->insert(
				$this->makeTableNameComponents('Ms_Users_Remembered'),
				[
					'user' => $userId,
					'selector' => $selector,
					'token' => $tokenHashed,
	...
	private function setRememberCookie($selector, $token, $expires) {
		...
		if (isset($selector) AND isset($token)) {
		...
		if (!isset($selector) OR !isset($token)) {
	...
	protected function onLoginSuccessful($userId, $email, $username, $status, $roles, $forceLogout, $remembered) {
		...
		try {
			$this->db->update(
				$this->makeTableNameComponents('Ms_Users'),
				[ 'last_login' => \time() ],
				[ 'id' => $userId ]
			);
		} catch (Error $e) {
	...
	public function confirmEmail($selector, $token) {
		...
		try {
			$confirmationData = $this->db->selectRow(
				'SELECT a.id, a.user_id, a.email AS new_email, a.token, a.expires, b.email AS old_email FROM ' . $this->makeTableName('Ms_Users_Confirmations') . ' AS a JOIN ' . $this->makeTableName('Ms_Users') . ' AS b ON b.id = a.user_id WHERE a.selector = ?',
				[ $selector ]
			);
		}
		catch (Error $e) {
		...
		if (!empty($confirmationData)) {
			if (\password_verify($token, $confirmationData['token'])) {
				if ($confirmationData['expires'] >= \time()) {
					...
					try {
						$this->db->delete(
							$this->makeTableNameComponents('Ms_Users_Resets'),
					...
					try {
						$this->db->delete(
							$this->makeTableNameComponents('Ms_Users_Confirmations'),
		...
		if ($this->isLoggedIn()) {
			...
			try {
				$verified = $this->db->selectValue(
					'SELECT verified FROM ' . $this->makeTableName('Ms_Users') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
			}
			catch (Error $e) {
	...
	private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback) {
		try {
			$latestAttempt = $this->db->selectRow(
				'SELECT user_id, email FROM ' . $this->makeTableName('Ms_Users_Confirmations') . ' WHERE ' . $columnName . ' = ? ORDER BY id DESC LIMIT 1 OFFSET 0',
				[ $columnValue ]
			);
		}
		catch (Error $e) {
	 ...
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
	private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, $twoFactorValid = false, callable $onBeforeSuccess = null) {
		$this -> throttle([ 'enumerateUsers', $this->getIpAddress() ], 1, (60 * 60), 75);
		$this -> throttle([ 'attemptToLogin', $this->getIpAddress() ], 4, (60 * 60), 5, true);

		$columnsToFetch = [ 'id', 'email', 'password', 'verified', 'username', 'status', 'roles_mask', 'force_logout', 'idenity_number', 'first_name', 'last_name', 'gender', 'twofa_authentication', 'academic_info', 'phone_number', 'birthday', 'address', 'avatar', 'verification' ];
		...
		if (\password_verify($password, $userData['password'])) {
			...
			$obj2fa = json_decode($userData['twofa_authentication']);

			if ((int) $obj2fa -> status === 1 AND $twoFactorValid === false) {
				$_SESSION['AUTH-2FacSecretKey'] = (string) $obj2fa -> secret_key;
				throw new twoFactorAuthentication();
			}

			if ((int) $userData['verified'] === 1 AND (int) $userData['status'] === 0) {
				if (!isset($onBeforeSuccess) OR (\is_callable($onBeforeSuccess) AND $onBeforeSuccess($userData['id']) === true)) {
					...
					$objAcademic = json_decode($userData['academic_info']);
					if(isset($objAcademic -> tag)){ $_SESSION['academic_details']['tag'] = $objAcademic -> tag ?? NULL; }	
					...
					if($rememberDuration !== null){ $this -> createRememberDirective($userData['id'], $rememberDuration); }
                ...
                /*if ((string) $userData['type'] !== 'user'){
                    throw new UserTypeNotAcceptable();
                }*/
			}
		} else {
			$this->throttle([ 'attemptToLogin', $this->getIpAddress() ], 4, (60 * 60), 5, false);
	...
	private function getUserDataByEmailAddress($email, array $requestedColumns) {
		try {
			...
			$userData = $this->db->selectRow(
				'SELECT ' . $projection . ' FROM ' . $this->makeTableName('Ms_Users') . ' WHERE email = ?',
				[ $email ]
			);
		}
		catch (Error $e) {
	...
	private function getOpenPasswordResetRequests($userId) {
		try {
			$requests = $this->db->selectValue(
				'SELECT COUNT(*) FROM ' . $this->makeTableName('Ms_Users_Resets') . ' WHERE user = ? AND expires > ?',
				[
					$userId,
					\time()
				]
	...
	public function resetPassword($selector, $token, $newPassword) {
		...
		try {
			$resetData = $this->db->selectRow(
				'SELECT a.id, a.user, a.token, a.expires, b.email, b.resettable FROM ' . $this->makeTableName('Ms_Users_Resets') . ' AS a JOIN ' . $this->makeTableName('Ms_Users') . ' AS b ON b.id = a.user WHERE a.selector = ?',
				[ $selector ]
			);
		}
		catch (Error $e) {
		...
		if ($this->isLoggedIn()) {
			try {
				$enabled = $this->db->selectValue(
					'SELECT resettable FROM ' . $this->makeTableName('Ms_Users') . ' WHERE id = ?',
					[ $this->getUserId() ]
				);
	...
	public function isLoggedIn() {
		return isset($_SESSION) AND isset($_SESSION[self::SESSION_FIELD_LOGGED_IN]) AND $_SESSION[self::SESSION_FIELD_LOGGED_IN] === true;
	}
	...
	public function getUserId() {
		if (isset($_SESSION) AND isset($_SESSION[self::SESSION_FIELD_USER_ID])) {
	...
	/**
	 * Returns the currently signed-in user's 2fa secret key by reading from the session
	 *
	 * @return string the 2fa secret key
	 */
    public function misy2faSecretKey() {
		if (isset($_SESSION) AND isset($_SESSION['2fa']['secretKey'])){ return $_SESSION['2fa']['secretKey']; } else { return null; }
	}
	
	/**
	 * Returns the currently signed-in user's 2fa status by reading from the session
	 *
	 * @return int the 2fa status
	 */
    public function misy2faStatus() {
		if (isset($_SESSION) AND isset($_SESSION['2fa']['status'])){ return $_SESSION['2fa']['status']; } else { return 2; }
	}
	 ...
	 */
    public function misyEmail() {
		if (isset($_SESSION) AND isset($_SESSION['email'])){ return $_SESSION['email']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display username by reading from the session
	 *
	 * @return string the display username
	 */
    public function misyUsername() {
		if (isset($_SESSION) AND isset($_SESSION['username'])){ return $_SESSION['username']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's department id by reading from the session
	 *
	 * @return int the department id
	 */
    public function misyDepartmentId() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['department'])){ return $_SESSION['academic_details']['department']; } else { return 0; }
	}
	/**
	 * Returns the currently signed-in user's unit id by reading from the session
	 *
	 * @return int the unit id
	 */
    public function misyUnitId() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['unit'])){ return $_SESSION['academic_details']['unit']; } else { return 0; }
	}
	
	/**
	 * Returns the currently signed-in user's unit by reading from the session
	 *
	 * @return string the unit
	 */
    public function misyUnit() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['unit_name'])){ return $_SESSION['academic_details']['unit_name']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's department by reading from the session
	 *
	 * @return string the department
	 */
    public function misyDepartment() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['department_name'])){ return $_SESSION['academic_details']['department_name']; } else { return null; }
	}
	
	/**
	 * Returns the currently signed-in user's degree by reading from the session
	 *
	 * @return int the degree
	 */
    public function misyDegree() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['degree'])){ return $_SESSION['academic_details']['degree']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's semester by reading from the session
	 *
	 * @return int the semester
	 */
    public function misySemester() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['semester'])){ return $_SESSION['academic_details']['semester']; } else { return 0; }
	}

	/**
	 * Returns the currently signed-in user's arrival type by reading from the session
	 *
	 * @return string the degree
	 */
    public function misyArrivalType() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['arrivalType'])){ return $_SESSION['academic_details']['arrivalType']; } else { return null; }
	}
	 ...
	 * @return string the degree
	 */
    public function misyGrants() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['grants'])){ return json_encode($_SESSION['academic_details']['grants'], JSON_UNESCAPED_UNICODE); } else { return null; }
	}
	
	/**
	 * Returns the currently signed-in user's student status by reading from the session
	 *
	 * @return int the student status
	 */
    public function misyStudentStatus() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['status'])){ return $_SESSION['academic_details']['status']; } else { return 0; }
	}

	/**
	 * Returns the currently signed-in user's display first name by reading from the session
	 *
	 * @return string the display first name
	 */
	public function misyFirstName() {
		if (isset($_SESSION) AND isset($_SESSION['firstName'])){ return $_SESSION['firstName']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display last name by reading from the session
	 *
	 * @return string the display last name
	 */
	public function misyLastName() {
		if (isset($_SESSION) AND isset($_SESSION['lastName'])){ return $_SESSION['lastName']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display full name by reading from the session
	 *
	 * @return string the display full name
	 */
	public function misyFullName() {
		if (isset($_SESSION) AND isset($_SESSION['firstName']) AND isset($_SESSION['lastName'])){ return $_SESSION['firstName'] . ' ' . $_SESSION['lastName']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display academic tag by reading from the session
	 *
	 * @return string the display academic tag
	 */
	public function misyTag() {
		if (isset($_SESSION) AND isset($_SESSION['academic_details']['tag'])){ return $_SESSION['academic_details']['tag']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display shortcut in full name by reading from the session
	 *
	 * @return string the display shortcut full name
	 */
	public function misyShortCutFullName() {
		if (isset($_SESSION) && isset($_SESSION['firstName']) AND isset($_SESSION['lastName'])) {
			$fullname = explode(" ", $_SESSION['firstName'] . ' ' . $_SESSION['lastName']);
			$cut = '';
	
			foreach ($fullname as $data) {
				$cut .= mb_substr($data, 0, 1, 'UTF-8');
			}
	
			return mb_strtoupper($cut, 'UTF-8');
		} else {
			return null;
		}
	}
	...
	/**
	 * Returns the currently signed-in user's display avatar by reading from the session
	 *
	 * @return string the display avatar
	 */
	public function misyAvatar() {
		if (isset($_SESSION) AND isset($_SESSION['avatar'])){ return $_SESSION['avatar']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display idenity by reading from the session
	 *
	 * @return string the display idenity
	 */
	public function misyIdenityNumber() {
		if (isset($_SESSION) AND isset($_SESSION['idenity'])){ return $_SESSION['idenity']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display phone number by reading from the session
	 *
	 * @return string the display phone number
	 */
	public function misyPhoneNumber() {
		if (isset($_SESSION) AND isset($_SESSION['phoneNumber'])){ return $_SESSION['phoneNumber']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display birthday by reading from the session
	 *
	 * @return string the display birthday
	 */
	public function misyBirthDay() {
		if (isset($_SESSION) AND isset($_SESSION['birthday'])){ return $_SESSION['birthday']; } else { return null; }
	}
	 ...
	 * @return string the display birthday
	 */
	public function misyGender() {
		if (isset($_SESSION) AND isset($_SESSION['gender'])){ return $_SESSION['gender']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display address by reading from the session
	 *
	 * @return string the display address
	 */
	public function misyAddress() {
		if (isset($_SESSION) AND isset($_SESSION['address'])){ return $_SESSION['address']; } else { return null; }
	}

	/**
	 * Returns the currently signed-in user's display contracts by reading from the session
	 *
	 * @return int the display contracts
	 */
	public function misyContractsVerify() {
		if (isset($_SESSION) AND isset($_SESSION['contractsVerify'])){ return $_SESSION['contractsVerify']; } else { return 0; }
	}
	/**
	 * Returns the currently signed-in user's display start up by reading from the session
	 *
	 * @return int the display start up
	 */
	public function misyIdenityVerify() {
		if (isset($_SESSION) AND isset($_SESSION['idenityVerify'])){ return $_SESSION['idenityVerify']; } else { return 0; }
	}

	/**
	 * Returns the currently signed-in user's display start up info by reading from the session
	 *
	 * @return int the display start up info
	 */
	public function misyInfoVerify() {
		if (isset($_SESSION) AND isset($_SESSION['infoVerify'])){ return $_SESSION['infoVerify']; } else { return 0; }
	}

	/**
	 * Returns the currently signed-in user's display start up info by reading from the session
	 *
	 * @return int the display balance
	 */
	public function misyBalance() {
		if (isset($_SESSION) AND isset($_SESSION['balance'])){ return $_SESSION['balance']; } else { return 0; }
	}

	/**
	 * Returns the currently signed-in user's display card status by reading from the session
	 *
	 * @return int the display card status
	 */
	public function misyCardVerify() {
		if (isset($_SESSION) AND isset($_SESSION['cardVerify'])){ return $_SESSION['cardVerify']; } else { return 2; }
	}

	/**
	 * validate mail
	 */
	public function misyValidateEmail($email) {
        if(preg_match("/^[a-zA-Z0-9._%+-]+@+[a-zA-Z]+(\.[a-zA-Z]{2,})+(\.[a-zA-Z]{2,3})?$/", $email) AND strlen($email) <= 64){ return true; } else { return false; }
	}
	...
	public function isNormal() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 0){ return true; } else { return false; }
	...
	public function isArchived() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 1){ return true; } else { return false; }
	...
	public function isBanned() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 2){ return true; } else { return false; }
	...
	public function isLocked() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 3){ return true; } else { return false; }
	...
	public function isPendingReview() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 4){ return true; } else { return false; }
	...
	public function isSuspended() {
		if (isset($_SESSION) AND isset($_SESSION['status']) AND $_SESSION['status'] == 5){ return 1; }
	...
	public function hasRole($role) {
		...
		if (isset($_SESSION) AND isset($_SESSION[self::SESSION_FIELD_ROLES])) {
			$role = (int) $role;
	...
	public function isRemembered() {
		if (isset($_SESSION) AND isset($_SESSION[self::SESSION_FIELD_REMEMBERED])) {
	...
	public function throttle(array $criteria, $supply, $interval, $burstiness = null, $simulated = null, $cost = null, $force = null) {
		...
		try {
			$bucket = $this->db->selectRow(
				'SELECT tokens, replenished_at FROM ' . $this->makeTableName('Ms_Users_Throttling') . ' WHERE bucket = ?',
				[ $key ]
			);
		}
		catch (Error $e) {
		...
		if (!$simulated) {
			...
			try {
				$affected = $this->db->update(
					$this->makeTableNameComponents('Ms_Users_Throttling'),
					$bucket,
					[ 'bucket' => $key ]
				);
			}
		...
		if ($this->isLoggedIn()) {
			...
			if (isset($existingSelector)) {
				...
				$existingExpiry = $this->db->selectValue(
					'SELECT expires FROM ' . $this->makeTableName('Ms_Users_Remembered') . ' WHERE selector = ? AND user = ?',
					[
						$existingSelector,
						$this->getUserId()
					]
